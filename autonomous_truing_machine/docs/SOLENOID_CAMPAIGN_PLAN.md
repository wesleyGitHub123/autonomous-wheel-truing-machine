...
- **Controls (Amendment 3, 2026-09-19 — reconciles the count):** 20 no-fire + 10 **air shots per station** = **40**, in **three groups judged on their own, never pooled**: `no_fire` (20), `air_LEFT` (10), `air_RIGHT` (10). "Per station" binds to the air shots only; that is what makes the earlier "in 40" true (20 + 10 + 10). The no-fire group has no station — nothing is excited — so it is one group of 20, tagged 10/10 by block only for bookkeeping. An air shot fires with the wheel rotated so the plunger lands between spokes; the rig is unchanged.
  - **Sequence:** interleaved within each station's block, order seeded, at most two of one kind in a row (`tools/campaign_sequence.py b30`). The plan's sha256 is stamped into every capture.
  - **Fixed n:** an excluded trial is replaced in place and ledgered, under pre-declared mechanical codes only — `ACK_REJECTED`, `NO_NEW_CAPTURE`, `FETCH_FAILED`, `CAPTURE_NOT_OK`, `RECORD_CONTRADICTS_REQUEST` — never on what the analysis made of it. The group sizes above are exact; a short or long group is **INCONCLUSIVE**, not a pass.
- **Automated spectra (Amendment 3):** from the firmware's own detector, before any clear or reject gate — see the rule below. Any line within ±5 Hz of 360 Hz is reported explicitly per group. **360 Hz lies inside the f1 band (350–600 Hz)**, so it is also judged by the general rule; and because the firmware selects the *lowest* strong peak in the band as f1, a persistent 360 Hz line would win that selection over a real spoke ring.
- **Pass (Amendment 3):** both, in **every** group.
  - **A — no false clear:** 0 clears (status `suspect`) in `no_fire`, `air_LEFT` and `air_RIGHT` alike. 0 of 40 bounds the false-clear rate below ≈7 % at 95 % confidence; per group only below ≈14 % (n = 20) and ≈26 % (n = 10).
  - **B — no coherent line:** a **line** is a *strong peak* the firmware detector reports in the f1 band whose **SNR reaches the chain profile's own `measurement_min_snr_db`** (12 dB, read from the data, not assumed). A line is **coherent** in a group if a peak within **±2 Hz** of it (the campaign's consistency band, reused) appears in **≥ 50 %** of that group's captures — 5 of 10, 10 of 20.
  - **Verdict:** FAIL if A or B is violated in any group; otherwise INCONCLUSIVE if any group is short, any capture has no readable spectrum, or the strong-peak list was cut off inside the f1 band; otherwise PASS. `tools/b30_verdict.py` computes it.
- **Why the groups are not pooled:** a coupling line specific to one actuator, present in all 10 of that station's air shots, is 25 % of 40 and would pass a pooled 50 % test while being exactly what B3.0 exists to catch.
- **Why the line is defined on SNR, not on the detector's peak list alone (Amendment 3):** measured on the repo's recorded captures, the ambient-noise capture yields 128 strong peaks — the list hits its cap — with prominences of 6–44 dB, because prominence on a zero-padded spectrum is dominated by null depth. A ±2 Hz recurrence rule over that list would call noise coherent. The firmware's own SNR statistic (peak magnitude against the median of the annulus 100–300 Hz away) separates them: the same noise capture peaks at 11.2 dB against the 12 dB gate, a hand pluck at 23.4 dB, a golden strike at 75.6 dB.
- **Why this is not redundant with false clears:** a false clear needs the onset's absolute floor (−55 dBFS rms), and the lowest-peak f1 selection, and the SNR gate. The line rule needs only the last. A hum below the onset floor never clears in a quiet control, and a line above the lowest noise peak is never selected; both would be latent hazards for a real strike. Because a quiet control is rejected at the onset stage with no spectrum, the analysis re-runs the onset with **only the absolute floor disabled** (the loudest event in the capture: the plunger impact for an air shot, the loudest noise frame for a no-fire), then windows the profile's full `window_ms` with no next-onset truncation. Detector, window and SNR statistic are the firmware's; a parity assertion checks the harness's strong-peak count against `truing_dsp_analyze_window`.
- **What a PASS does not mean:** it does not mean nothing is there. A line below the SNR gate, or one recurring in fewer than half a group, is not detected by this rule, and 20/10/10 captures cannot rule out a rare one. The stage report says so.
- **If it fails:** one discriminating block (the offending solenoid decoupled on foam vs mounted), fix, rerun. **(Amendment 3)** The failing group localises it: `no_fire` fails point at the shared path (mains, the supply, the mic chain); `air_LEFT` or `air_RIGHT` fails point at that actuator's mechanical coupling.
