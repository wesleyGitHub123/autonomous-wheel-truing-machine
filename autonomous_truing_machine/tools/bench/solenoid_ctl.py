...
L/R 500 ms hold, a alternating run, +/- shot width, {/} push ramp, [/] release ramp, k/K brake
duty, m/M brake ms, g/G catch duty, t/T catch delay, w/W catch width, s status, c/u/d/x manual
jog, ? help). The board's replies print here and are appended to a log file, together with
anything you note, so what you saw and what the board says it did end up side by side in one
timestamped record.
...
emit("keys go to the board: l r b v L R a + - { } [ ] k K m M g G t T w W s c u d x ?"
     "   |   n = note, Esc = quit\n")
try:
    while True:
        ch = msvcrt.getwch()
